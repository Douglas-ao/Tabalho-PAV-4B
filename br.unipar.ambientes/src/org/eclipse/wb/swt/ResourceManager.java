...
public class ResourceManager extends SWTResourceManager {
	...
	/**
	 * Obtém uma {@link Image} baseado no {@link Bundle} e no caminho informado
	 * 
	 * @param path o caminha da pasta onde está a imagem
	 * @return a {@link Image} encontrada
	 * @author Filipe Wutzke
	 */
	public static Image getPluginImage(String path) {
		try {
			URL url = getPluginImageURL(SYMBOLIC_NAME, ICON_ROOT_PATH + path);
			if (url != null) {
				return getPluginImageFromUrl(url);
			}
		} catch (Throwable e) {
	...
	private static Image getPluginImageFromUrl(URL url) {
		try {
			...
			if (image == null) {
				try (InputStream stream = url.openStream();) {//NOSONAR
	...
	/**
	 * Retorna uma {@link ImageDescriptor} baseada no {@link Bundle} no caminho da pasta informado.
	 * 
	 * @param path o caminho da pasta onde encontra-se a imagem
	 * @return a {@link ImageDescriptor} encontrada
	 * @author Filipe Wutzke
	 */
	public static ImageDescriptor getPluginImageDescriptor(String path) {
		try {
			path = ICON_ROOT_PATH + path;
			URL url = getPluginImageURL(SYMBOLIC_NAME, path);
			if (url != null) {
				return ImageDescriptor.createFromURL(url);
			}
		} catch (Throwable e) {
